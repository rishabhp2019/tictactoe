...
players() {
  ...
  read name2
  echo " "
  echo "Either 0 or 1 will be drawn. If 0 is drawn then $name1 is X and goes first. If 1 is drawn then $name2 is X and goes first."
...
game_tie() {
  ...
  if [[ ${array[0]} = - || ${array[1]} = - || ${array[2]} = - || ${array[3]} = - || ${array[4]} = - || ${array[5]} = - || ${array[6]} = - || ${array[7]} = - || ${array[8]} = - || ${array[9]} = - ]]; then
    ...
    tie=1
    echo "The game is a tie! Nobody Wins! Would you like to do a rematch?"
    read answer
    if [ $answer = y -o Y ]; then
      game_rematch
    elif [ $answer = n -o N ]; then
    exit
    fi
  fi
}
...
turns() {
  while true; do
    while true; do
      echo -e "$p1 please enter your input (1-9) --> \c"
      read input
      i=$((input - 1))
      if [ ${array[$i]} == - ]; then
        array[$i]='X'
    ...
    game_tie
    while true; do
      echo -e "$p2 please enter your input (1-9) --> \c"
      read input
      i=$((input - 1))
      if [ ${array[$i]} == - ]; then
        array[$i]='O'
